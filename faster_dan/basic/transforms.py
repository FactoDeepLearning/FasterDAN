...
import cv2
import math
from faster_dan.basic.utils import randint, rand_uniform, rand
...
def get_list_augmenters(img, aug_configs, fill_value):
    ...
    for aug_config in aug_configs:
        ...
        if aug_config["type"] == "dpi":
        ...
        elif aug_config["type"] == "rotate":
            augmenters.append(RandomRotation(degrees=aug_config["max_degree"], expand=aug_config["keep_all"], fill=fill_value))
            ...
            new_w, new_h = [int(t * aug_config["ratio"]) for t in img.size]
            augmenters.append(RandomCrop((new_h, new_w)))

        elif aug_config["type"] == "fixed_crop":
            augmenters.append(RandomCrop((aug_config["h"], aug_config["w"])))

        else:
            print("Error - unknown augmentor")
            exit(-1)
