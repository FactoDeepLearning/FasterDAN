...
from faster_dan.basic.transforms import apply_data_augmentation
...
class GenericDataset(Dataset):
    ...
    def __init__(self, params, set_name, custom_name, paths_and_sets):
        ...
        if "balance_datasets" in params["config"] and params["config"]["balance_datasets"] and set_name == "train":
            dataset_names = list(params["datasets"].keys())
            num_datasets = len(dataset_names)
            samples_by_dataset = dict()
            for name in dataset_names:
                foldername = os.path.basename(params["datasets"][name])
                samples_by_dataset[name] = [s for s in self.samples if foldername in s["name"]]
            max_len = max([len(samples_by_dataset[k]) for k in samples_by_dataset])
            new_samples = np.zeros((num_datasets*max_len), dtype=object)
            for i in range(num_datasets):
                dataset_samples = samples_by_dataset[dataset_names[i]]
                if len(dataset_samples) < max_len:
                    mul = max_len // len(dataset_samples)
                    rem = max_len % len(dataset_samples)
                    dataset_samples = np.repeat(dataset_samples, mul if rem == 0 else mul+1)[:mul*len(dataset_samples)+rem]
                new_samples[i::num_datasets] = dataset_samples
            ...
            with open(os.path.join(path, "labels.pkl"), "rb") as f:
                ...
                for filename in natural_sort(gt.keys()):
                    ...
                    if type(gt[filename]) is dict:
                        ...
                        if "languages" in gt[filename].keys():
                            samples[-1]["languages"] = gt[filename]["languages"]
                        if "scripts" in gt[filename].keys():
                            samples[-1]["scripts"] = gt[filename]["scripts"]
                        if "category" in gt[filename].keys():
                            samples[-1]["category"] = gt[filename]["category"]
        return samples
