...
from faster_dan.basic.metric_manager import MetricManager
from faster_dan.basic.scheduler import DropoutScheduler
...
class GenericTrainingManager:
    ...
    def init_hardware_config(self):
        ...
        self.params["model_params"]["device"] = self.device if isinstance(self.device, str) else self.device.type
        # Print GPU info
        # global
        print("##################")
    ...
    def load_model(self, reset_optimizer=False, strict=True):
        ...
        if checkpoint and "charset" in checkpoint and "keep_charset_from_checkpoint" in self.params["model_params"] and self.params["model_params"] ["keep_charset_from_checkpoint"]:
            self.params["model_params"]["vocab_size"] = len(checkpoint["charset"])
        # Instantiate Model
        ...
        if "dropout_scheduler" in self.params["model_params"]:
            ...
            if checkpoint:
                self.dropout_scheduler.step(checkpoint["step"])
    ...
    def save_params(self):
        ...
        path = os.path.join(self.paths["results"], "params.txt")
        i = 1
        while os.path.isfile(path):
            path = "{}_{}.txt".format(os.path.join(self.paths["results"], "params"), i)
            i += 1
    ...
    def train(self):
        ...
        min_epochs_before_valid = 0 if "start_valid_from_epochs" not in self.params["training_params"] else self.params["training_params"]["start_valid_from_epochs"]
        min_steps_before_valid = 0 if "start_valid_from_steps" not in self.params["training_params"] else self.params["training_params"]["start_valid_from_steps"]
        ...
        for num_epoch in range(self.latest_epoch+1, nb_epochs):
            ...
            if min_epochs_before_valid < self.latest_epoch and min_steps_before_valid < self.latest_step and self.params["training_params"]["eval_on_valid"] and num_epoch % self.params["training_params"]["eval_on_valid_interval"] == 0:
