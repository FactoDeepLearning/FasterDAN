...
class READ2016DatasetFormatter(OCRDatasetFormatter):
    def __init__(self, level, set_names=["train", "valid", "test"], dpi=150, end_token=False, sem_token=False):
        ...
        self.map_datasets_files.update({
            "READ_2016": {
                ...
                # (8,349 for train, 1,040 for validation and 1,138 for test)
                "line": {
                    "arx_files": ["Test-ICFHR-2016.tgz", "Train-And-Val-ICFHR-2016.tgz"],
                    "needed_files": [],
                    "format_function": self.format_read2016_line,
                },
                # (1,584 for train, 179 for validation and 197 for test)
                "paragraph": {
                    "arx_files": ["Test-ICFHR-2016.tgz", "Train-And-Val-ICFHR-2016.tgz"],
                    "needed_files": [],
                    "format_function": self.format_read2016_paragraph,
                },
    ...
    def format_read2016_line(self):
        dataset = self.preformat_read2016()
        for set_name in ["train", "valid", "test"]:
            i = 0
            for page in dataset[set_name]:
                page_img = np.array(Image.open(page["img_path"]))
                for text_region in page["text_regions"]:
                    for line in text_region["lines"]:
                        ...
                        line_img = page_img.copy()[line["coords"]["top"]:line["coords"]["bottom"], line["coords"]["left"]:line["coords"]["right"]]
                        line_img = self.resize(line_img, 300, self.dpi)
                        Image.fromarray(line_img).save(new_img_path)
                        self.charset = self.charset.union(set(line["label"]))
                        self.gt[set_name][new_img_name] = {
                            "text": line["label"],
                        }
                        i += 1

    def format_read2016_paragraph(self):
        dataset = self.preformat_read2016()
        for set_name in ["train", "valid", "test"]:
            i = 0
            for page in dataset[set_name]:
                ...
                for text_region in page["text_regions"]:
                    new_img_name = "{}_{}.jpeg".format(set_name, i)
                    new_img_path = os.path.join(self.target_fold_path, set_name, new_img_name)
                    self.charset = self.charset.union(set(text_region["label"]))
                    region_top = text_region["coords"]["top"]
                    region_bottom = text_region["coords"]["bottom"]
                    region_left = text_region["coords"]["left"]
                    region_right = text_region["coords"]["right"]
                    region_img = page_img.copy()[region_top:region_bottom, region_left:region_right]
                    region_img = self.resize(region_img, 300, self.dpi)
                    Image.fromarray(region_img).save(new_img_path)
                    lines = list()
                    for line in text_region["lines"]:
                        lines.append({
                            "text": line["label"],
                            "top": line["coords"]["top"] - region_top,
                            "bottom": line["coords"]["bottom"] - region_top,
                            "left": line["coords"]["left"] - region_left,
                            "right": line["coords"]["right"] - region_left,
                        })
                        lines[-1] = self.adjust_coord_ratio(lines[-1], self.dpi / 300)
                    self.gt[set_name][new_img_name] = {
                        "text": text_region["label"],
                        "lines": lines,
                        "nb_cols": 1,
                    }
                    i += 1
    ...
    def format_read2016_page(self):
        ...
        for set_name in ["train", "valid", "test"]:
            for i, page in enumerate(dataset[set_name]):
                ...
                if self.sem_token:
                    if self.end_token:
                        new_label = self.tokens["page"] + new_label + self.tokens["page"].upper()
                    else:
                        new_label = self.tokens["page"] + new_label
    ...
    def format_read2016_double_page(self):
        ...
        for set_name in ["train", "valid", "test"]:
            ...
            for document in dataset[set_name]:
                ...
                if self.sem_token:
                    if self.end_token:
                        document_label = self.tokens["page"] + label_left + self.tokens["page"].upper() + self.tokens["page"] + label_right + self.tokens["page"].upper()
                    else:
                        document_label = self.tokens["page"] + label_left + self.tokens["page"] + label_right
                else:
    ...
    def add_tokens_in_charset(self):
        ...
        if self.sem_token:
            ...
            all_tokens = begin_tokens + [b.upper() for b in begin_tokens]
            if self.end_token:
                self.charset = self.charset.union(set(all_tokens))
            else:
                self.charset = self.charset.union(set(begin_tokens))
    ...
    def group_by_page_number(self, dataset):
        ...
        for set_name in ["train", "valid", "test"]:
            for page in dataset[set_name]:
                page_num = int(page["text_regions"][0]["label"].replace(".", "").replace(" ", "").replace(self.tokens["number"], "").replace(self.tokens["number"].upper(), ""))
    ...
    def update_label(self, label, start_token):
        ...
        if self.sem_token:
            if self.end_token:
                return start_token + label + start_token.upper()
            else:
    ...
    def sort_text_regions(self, text_regions, page_width):
        ...
        for group in ordered_groups:
            ...
            if len(text_regions) == 1 and text_regions[0]["label"].replace(".", "").replace(" ", "").isdigit():
                ...
                sorted_text_regions[-1]["label"] = self.update_label(sorted_text_regions[-1]["label"], self.tokens["number"])
            else:
                ...
                for i, text_region in enumerate(sorted(left, key=lambda tr: tr["coords"]["top"])):
                    ...
                    sorted_text_regions[-1]["label"] = self.update_label(sorted_text_regions[-1]["label"], self.tokens["annotation"])
                    if i == 0 and self.sem_token:
                        sorted_text_regions[-1]["label"] = self.tokens["section"] + sorted_text_regions[-1]["label"]
                for i, text_region in enumerate(sorted(right, key=lambda tr: tr["coords"]["top"])):
                    sorted_text_regions.append(text_region)
                    sorted_text_regions[-1]["mode"] = "body"
                    sorted_text_regions[-1]["label"] = self.update_label(sorted_text_regions[-1]["label"], self.tokens["body"])
                    if i == 0 and self.sem_token and len(left) == 0:
                        sorted_text_regions[-1]["label"] = self.tokens["section"] + sorted_text_regions[-1]["label"]
                    if i == len(right)-1 and self.sem_token and self.end_token:
                        sorted_text_regions[-1]["label"] = sorted_text_regions[-1]["label"] + self.tokens["section"].upper()
